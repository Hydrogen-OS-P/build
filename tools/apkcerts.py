...
def getName(codePath):
    ...
    apkname = os.path.basename(codePath)
...
def main():
    ...
    sigpkgs = {}
    for pkg in packages:
        name = getName(pkg.getAttribute("codePath"))
        cert= pkg.getElementsByTagName("cert")
        if not cert or not name:
            print "WARNNING: no codePath or cert of %s " % pkg.toxml()
            continue
    ...
    with open(sys.argv[2], "w") as f:
        for keyindex, pkgnames in sigpkgs.items():
            ...
            for pkgname in pkgnames:
                if sig:
                    line = 'name="{0}" certificate="build/security/{1}.x509.pem" private_key="build/security/{1}.pk8"\n'.format(pkgname, sig)
                else:
